...
async function addSourceFiles(project: Project) {
  ...
  const filePaths = excludeFiles(exFileS)
  ...
  await Promise.all([
    ...
    ...epPaths.map(async (file) => {
      const content = await readFile(path.resolve(dealAntUiDir, file), 'utf-8')
      sourceFiles.push(project.createSourceFile(path.resolve(pkgRoot, file), content))
    }),
  ])
  return sourceFiles
}
export default series(withTaskName('generateType', generateType))
